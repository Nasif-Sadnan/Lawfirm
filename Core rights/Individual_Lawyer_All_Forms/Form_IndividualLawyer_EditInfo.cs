...
{
    public partial class Form_IndividualLawyer_EditInfo : Form
    {
        ...
        public Form_IndividualLawyer_EditInfo(string ID)
        {
            InitializeComponent();
            this.firmID = ID;
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                TxT_Box_Judicial_Licence_num.Text = reader["Judicial licence number "].ToString();
                            }
                            else
                            {
        ...
        private void Icn_Pic_Edit_Done_Click(object sender, EventArgs e)
        {
            ...
            string Judicial_Licence_Number = TxT_Box_Judicial_Licence_num.Text;

            string updateQuery = "UPDATE [Individual Lawyer] SET Password = @Password, Division = @Division, District = @District, Gmail = @Gmail, [Judicial licence number ] = @Judicial_Licence_Number WHERE [Lawyer ID ] = @LawyerID";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(updateQuery, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@Judicial_Licence_Number", Judicial_Licence_Number);
                    ...
                    try
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            Form_IndividualLawyer_ShowInfo showInfoForm = new Form_IndividualLawyer_ShowInfo(firmID, this);

                            this.Hide();

                            Form_Individual_Lawyer_DashBoard_ShowInfo parentForm = this.ParentForm as Form_Individual_Lawyer_DashBoard_ShowInfo;
