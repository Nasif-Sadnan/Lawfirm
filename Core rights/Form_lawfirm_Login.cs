...
{
    public partial class Form_lawfirm_Login : Form
    {
        ...
        {
            TxT_Box_ID.Text = "Enter your Law Firm ID";
        ...
        {
            if (TxT_Box_ID.Text == "Enter your Law Firm ID")
            {
                TxT_Box_ID.Text = "";
        ...
        {
            Form_LawFirm_Registration f_LawFirm_Registration = new Form_LawFirm_Registration(this);
            this.Hide();
            f_LawFirm_Registration.ShowDialog();
        }

        private void LbL_Reset_Pass_Click(object sender, EventArgs e)
        {
            if (TxT_Box_ID.Text == "Enter your Law Firm ID" || TxT_Box_ID.Text == "")
            {
                MessageBox.Show("Please enter your Law Firm ID");
            }
            else
            {
                string firmID = TxT_Box_ID.Text;

                if (CheckIfFirmIDExists(firmID))
                {
                    Firm_ID = firmID;
                    Form_LawFirm_SendMail_ForPassCHnage form_LawFirm_SendMail_ForPassCHnage = new Form_LawFirm_SendMail_ForPassCHnage(Firm_ID);
                    this.Hide();
                    form_LawFirm_SendMail_ForPassCHnage.ShowDialog();
                }
                else
                {
                    MessageBox.Show("The Firm ID is not available, please enter a registered Firm ID");
                }
            }
        }

        private bool CheckIfFirmIDExists(string firmID)
        {
            bool firmIDExists = false;
            ...
            using (SqlConnection connection = new SqlConnection(@"Data Source=LAPTOP-368TJ83Q\SQLEXPRESS;Initial Catalog=""Core Rights"";Integrated Security=True"))
            {
                ...
                string query = $"SELECT COUNT(*) FROM [Law Firm] WHERE Firm_ID = @FirmID";
        ...
        private void Icn_BtN_Register_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    string query= "SELECT * FROM [Law Firm] WHERE Firm_ID ='" + TxT_Box_ID.Text + "' AND Password='" + TxT_Box_Password.Text + "'";
                    ...
                    if(dt.Rows.Count > 0)
                    {
                        ...
                        Form_Home_LawFirm form_Home_LawFirm = new Form_Home_LawFirm(userID);

                        this.Hide();
                        form_Home_LawFirm.ShowDialog();
                    }
                    else
                    {
