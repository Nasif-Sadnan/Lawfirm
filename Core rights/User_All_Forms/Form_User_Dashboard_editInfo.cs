...
{
    public partial class Form_User_Dashboard_editInfo : Form
    {
        ...
        public Form_User_Dashboard_editInfo(string ID)
        {
            InitializeComponent();
            this.firmID = ID;
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                TxT_Box_MobileNo.Text = reader["Mobile_NUmber"].ToString();
                                Date_TimePick_DOB.Text= reader["DOB"].ToString();
                            }
                            else
                            {
        ...
        private void Icn_Pic_Edit_Done_Click(object sender, EventArgs e)
        {
            ...
            string newMobileno=TxT_Box_MobileNo.Text;
            string newDOB=Date_TimePick_DOB.Text;


            string updateQuery = "UPDATE [User] SET Password = @Password, Division = @Division, District = @District, Gmail = @Gmail, Mobile_NUmber = @MobileNumber, DOB = @DOB WHERE NID = @FirmID";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(updateQuery, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MobileNumber", newMobileno);
                    ...
                    try
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            Form_User_DashBoard_showinfo showInfoForm = new Form_User_DashBoard_showinfo(firmID, this);

                            this.Hide();

                            Form_User_DasBoard parentForm = this.ParentForm as Form_User_DasBoard;
